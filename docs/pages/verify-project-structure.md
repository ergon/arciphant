...
Arciphant provides the task `validatePackageStructure` verify correct package structure. The task is only available in
the root project but scans all the subprojects, too.
...
The `packageStructureValidation` task provides the following configuration options (see also
