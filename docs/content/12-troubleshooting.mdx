...
[`projectDependencies` task](/tasks#projectdependencies).

Configuration errors detected by Arciphant fail the build during the initialization phase with a message prefixed
`Arciphant configuration error: …` — these indicate a problem in the `arciphant { }` block itself. The most common
errors and symptoms:
...
block. Add one of the convention plugins with `apply false` to `settings.gradle.kts` — see
...
```text
Arciphant configuration error: Component with name 'web' has already been declared.
```
...
```text
Arciphant configuration error: Component with name 'web' does not exist.
```
...
- `plugin` (on components and bundles) is only available in the **project** layout — see
  [Layout-specific restrictions](/component-layouts#layout-specific-restrictions).
