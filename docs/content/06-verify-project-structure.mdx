...
Arciphant provides the task `validatePackageStructure` to verify correct package structure. The task is only available in
...
| `excludeResourcesFolder` | By default, all folders in the `src`-folder of each project are validated.<br />Use `excludeResourcesFolder()` to exclude the resource folder (`src/main/resources`) from validation. |
| `excludeSrcFolders` | By default, all folders in the src-folder of each project are validated.<br />Use `excludeSrcFolders()` to exclude specific folders.<br />Example: To exclude `src/main/generated` use: `excludedSrcFolder("main/generated")` |
