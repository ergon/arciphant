...
components' characteristics and external dependencies.
...
Unfortunately, these plugins won't work together with Arciphant out-of-the-box. The problem is that the
`pluginManagement` block is only honored by Gradle when plugins are applied in the `plugins` block. Since Arciphant
