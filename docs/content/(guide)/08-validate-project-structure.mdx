...
Configure mappings for specific project names. The name can be either a leaf project (e.g., an arciphant
component) or a parent project (e.g., an arciphant module). In the source set layout, component names are mapped the
same way, even though components are not projects there. The `basePackageName` is still used. The configured value
replaces only the package fragment related to the specified project or component.
