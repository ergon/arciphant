...
<Frame caption="Four modules sharing the same clean-architecture-like component structure.">

![Four modules with the components domain, application, web and db](../images/structure-01-modules.svg)
...
The above configuration creates the following structure and sets up the dependencies between the components (domain,
application, web, db) in each module. How the components are mapped to Gradle depends on the configured
[component layout](/component-layouts):

<Tabs>
  <Tab title="Project layout">
    Every component becomes its own nested Gradle project (<Icon class="inline" name="package" size={15} />) with the
    standard Gradle source sets (<Icon class="inline" name="folder-code" size={15} />):

    <include>./_folder-tree/folder-tree-project-layout.mdx</include>

    `src/testFixtures` is only present once the optional `java-test-fixtures` plugin is applied — see
    [Structure test code](#structure-test-code).
  </Tab>
  <Tab title="Source set layout">
    Every module becomes one Gradle project (<Icon class="inline" name="package" size={15} />) whose components are
    source sets (<Icon class="inline" name="folder-code" size={15} />):

    <include>./_folder-tree/folder-tree-source-set-layout.mdx</include>

    The `…Test` and `…TestFixtures` source sets are created by default; they can be renamed or disabled — see
    [Component layouts](/component-layouts) for the details.
  </Tab>
</Tabs>
...
<Frame caption="A shared library module with the same component structure (shown with two modules for simplicity).">

![Two modules whose components depend on the same-named components of a shared library module](../images/structure-02-library.svg)
...
Each component of each domain module automatically gets an <Tooltip headline="api dependency" tip="A transitive dependency: whatever depends on the component also gets access to the shared library code." cta="Dependency types" href="#dependency-types">`api` dependency</Tooltip> on the same-named component in the library
module.

<Expandable title="Complete settings.gradle.kts">
...
:::info[Shared libraries for independent modules?]
Whether a shared library is reasonable for independent domain modules may vary from case to case.
It is probably more accepted in case of a modulith than with microservices. Again, Arciphant provides the ability
without making a statement whether to use it or not.
:::
...
<Frame caption="Modules C and D with an additional `fs` component.">

![Modules C and D with an additional fs component, modules A and B without](../images/structure-03-different-shapes.svg)
...
<Frame caption="Module D with an individual `ext-api` component.">

![Module D with an individual ext-api component](../images/structure-04-individual-shapes.svg)
...
This creates a module `my-app` that has a dependency to all components of all <Tooltip headline="Functional module" tip="Domain and library modules — modules that consist of components. Bundle modules are not functional modules." cta="Concepts" href="/concepts#module">functional modules</Tooltip>. The bundle module itself does not
...
automatically depend on all functional modules:
...
This describes the default project layout. In the source set layout, Arciphant creates dedicated production,
test-fixtures, and test source sets itself; the `java-test-fixtures` plugin is not required.
See [Component Layouts](/component-layouts#test-source-set-configuration) for configuration details.
:::

## Debug project dependencies

:::tip[Print the wired dependencies]
To verify whether the dependencies Arciphant wired are as expected, run `./gradlew -q projectDependencies` — see
[Gradle tasks](/tasks#projectdependencies). The task only prints dependencies between Gradle projects; in the source
set layout, the component wiring inside a module does not appear.
:::
