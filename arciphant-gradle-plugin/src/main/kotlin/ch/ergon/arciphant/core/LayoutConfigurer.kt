...
/**
 * Base of the layout-specific configurers, which apply the configuration derived from the Arciphant
 * settings to the Gradle projects. Projects that are not managed by Arciphant are ignored.
 *
 * For every managed project, the configurer first registers the `component` dependency notation extension,
 ...
 */
internal abstract class LayoutConfigurer(projectConfigs: List<GradleProjectConfig>) {
    ...
    fun configure(project: Project) {
        ...
        doConfigure(project, config)
    }

    protected abstract fun componentDependencyFactory(project: Project): ComponentDependencyFactory

    protected abstract fun doConfigure(project: Project, config: GradleProjectConfig)
}
