...
open class ArciphantDsl {
    ...
    fun library(name: String, basePath: String? = null, template: ModuleTemplateBuilder) =
        library(name, basePath, setOf(template))

    fun library(
        name: String,
        basePath: String? = null,
        templates: Set<ModuleTemplateBuilder> = emptySet()
    ): FunctionalModuleBuilder {
        verifyName(name, "library")
        return FunctionalModuleBuilder(
            name,
            basePath,
            templates,
            FunctionalModuleType.LIBRARY
        ).also { functionalModules.add(it) }
    }

    fun module(name: String, basePath: String? = null, template: ModuleTemplateBuilder) =
        module(name, basePath, setOf(template))
